...
public class AStarUtil {
    ...
    public boolean is_valid(int x, int y) {
        if(x > 1580 || y > 880 || x < 20 || y < 20) return false;
        if(map[x+20][y+20] == -1 || map[x-20][y-20] == -1 || map[x+20][y-20] == -1 || map[x-20][y+20] == -1 || 
           map[x][y+20] == -1 || map[x][y-20] == -1 || map[x+20][y] == -1 || map[x-20][y] == -1) return false;
        if ((x >= (800+R-40)) ||
           (y >= (0.5625*(800+R)-40))  ||
           (x <= (800-R+40) ) ||
           (y <= (0.5625*(800-R)+40)))   return false;
        // 如果结点的位置是-1，则不合法
        try{
