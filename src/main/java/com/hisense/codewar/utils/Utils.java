...
//		System.out.println(bearing);
	...
	public static Position getNextBulletByTick(int nowX, int nowY, int r, int tick) {
		...
		int x = Utils.doubleToInt((nowX + AppConfig.BULLET_SPEED * Math.cos(rr) * tick));
		// starty + kProjectileSpeed * sinf(rr) * elapsed;
		int y = Utils.doubleToInt((nowY + AppConfig.BULLET_SPEED * Math.sin(rr) * tick));
...
//		return distance < totalR;
		//考虑误差，
		int blockWidthBigger = blockWidth + 2;
		int minX = blockX - blockWidthBigger;
		int maxX = blockX + blockWidthBigger;
		int minY = blockY - blockWidthBigger;
		int maxY = blockY + blockWidthBigger;
